...
const onSubmit = async (data: FormData) => {
  ...
  try {
    // Stage 1: Call signInWithOtp which triggers OTP email
    await otpService.sendLoginOtp(data.email);
...
const onVerifyOtp = async (token: string) => {
  ...
  try {
    ...
    const { session } = await otpService.verifyLoginOtp(userEmail, token);
    
    if (!session) {
      throw new Error("Verification failed. Please try again.");
    }

    // Stage 3: Finalize session
    const { role } = await finalizeSession(session, "password");

    // Stage 4: Sync with backend (requirement)
...
const onResendOtp = async () => {
  if (!loginData) return;
  await otpService.sendLoginOtp(loginData.email);
};

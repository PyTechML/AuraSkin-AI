    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="border-border">
            <CardHeader>
              <CardTitle className="font-heading text-lg">Order timeline</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                {TIMELINE_STEPS.map((step, i) => {
                  ...
                  return (
                    <div key={step} className="flex items-center flex-shrink-0">
                      ...
                      <span className={`ml-1 text-xs hidden sm:inline ${reached ? "text-foreground" : "text-muted-foreground"}`}>
                        {step.replace(/_/g, " ")}
                      </span>
                      {i < TIMELINE_STEPS.length - 1 && (
                        <div className={`w-4 sm:w-8 h-0.5 mx-1 ${reached ? "bg-accent" : "bg-muted"}`} />
                      )}
                    </div>
                  );
                })}
              ...
              {order.deliveryEstimate && (
                <p className="text-sm text-muted-foreground mt-2">Est. delivery: {order.deliveryEstimate}</p>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4">
              ...
              <form onSubmit={handleAddNote} className="flex gap-2">
                <Input
                  ...
                  className="flex-1"
                />
                <Button type="submit" size="sm" disabled={updating || !noteInput.trim()}>
                  Add
                </Button>
              </form>
            </CardContent>
          ...
          <Card className="border-border">
            <CardHeader>
              <CardTitle className="font-heading text-lg">Customer communication log</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-muted-foreground">
                Order confirmation and status emails are sent automatically. No manual messages yet.
              </p>
            </CardContent>
          </Card>
        ...
        <div className="space-y-6">
          <Card className="border-border sticky top-24">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span>{(order.paymentStatus ?? "paid").replace(/_/g, " ")}</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-border">
            <CardHeader>
              <CardTitle className="font-heading text-lg">Shipping tracking</CardTitle>
            </CardHeader>
          ...
          {!showRefundPanel && order.status !== "cancelled" && order.status !== "delivered" && (
            <Card className="border-border">

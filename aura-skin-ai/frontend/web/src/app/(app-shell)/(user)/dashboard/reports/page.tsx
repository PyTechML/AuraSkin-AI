...
export default function ReportsPage() {
  ...
  const timelineReports: TimelineReport[] = sortReportsNewestFirst([
    ...reports.map((r) => ({ ...r, source: "AI Generated" as const })),
    ...manualReports,
  ]);
    ...
    <div className="space-y-6">
                ...
                <DialogFooter className="mt-2">
                  {latest ? (
                    <Button variant="outline" asChild>
                      <Link href={`/dashboard/reports/${latest.id}`}>View latest report</Link>
                    </Button>
                  ) : null}
                </DialogFooter>
              </DialogContent>
        ...
        <CardContent>
          {timelineReports.length === 0 ? (
            <div className="rounded-xl border border-border/60 bg-muted/40 p-6">
              <p className="font-heading text-lg font-semibold">No reports yet</p>
              <p className="text-sm text-muted-foreground mt-1">
                Start with a baseline assessment or add a manual entry to begin your clinical progress archive.
              </p>
              <div className="mt-4 flex flex-wrap gap-2">
                <Button asChild>
                  <Link href="/dashboard/assessment/start">Start assessment</Link>
                </Button>
      ...
      <Card className="border-border">
        ...
        <CardContent className="flex flex-wrap items-center gap-2">
          {nextStep.ctaHref === "/dashboard/reports" ? (
